...
func Project(in Input) Event {
	...
	case method == "turn/completed" || in.Status == string(domain.TaskCompleted):
		return Event{Kind: domain.SemanticTurnCompleted, Phase: "completed", DisplayText: "Codex 处理已结束", Visible: true}
	case strings.Contains(raw, "grok_search") || strings.Contains(raw, "grok"):
	...
	case strings.Contains(method, "item") && (strings.Contains(raw, "tool") || strings.Contains(raw, "command") || strings.Contains(raw, "function")):
		return Event{Kind: domain.SemanticToolActivity, Phase: "tool", DisplayText: "Codex tool activity", Visible: true}
	case strings.Contains(method, "item") && (strings.Contains(raw, "agent_message") || strings.Contains(raw, "assistant_message")):
		return Event{Kind: domain.SemanticAssistantMessage, Phase: "assistant", DisplayText: "Codex response updated", Visible: true}
	case strings.Contains(method, "question") || strings.Contains(raw, "question"):
		return Event{Kind: domain.SemanticQuestion, Phase: "input", DisplayText: "Codex 正在等待你的回复", Visible: true}
