...
const (
	...
	// 2026-08-24 换词换说法：钩子类型留下，开场切口必须换。中段顺序可跟对标文走。
	// 2026-08-24 禁写 50–77 万亿定存到期区间。流水线和过软共情不再当硬闸。
...
//
// rewrite（默认）= RewritePromptStamp（文案进化台 2026-08-24 钩子类型定稿）。
// 相对上一版加了这些刀：
//   - 六件套都要在，中后段出场顺序可以换；禁止提前揭答案、把课收到开头
//   - 钩子类型不许换：换现场和用词，不改成另一类更小的损失
//   - 未揭晓的答案必须出现在前 3 句内
//   - 禁止纯解释句、纯共情句开场
...
func buildWriterPrompt() string {
	...
	b.WriteString("写稿时这六条一条都不能丢。钩子类型（第1条）和未揭晓的答案（第2条）必须留在前 3 句，不许挪到中段。中后段证明、差距、情绪的出场顺序可以换：允许先算家里的账再落到全国数字，允许先讲历史对照再落到这次为什么来。禁止把后半段答案提前揭开，禁止把课收到开头。\n")
	b.WriteString("\n【硬性保留】\n")
	...
	b.WriteString("- 钩子类型不许换。原稿第一句靠「反常数字 / 未揭晓去向 / 对仗打脸」里的哪一种，新稿第一句必须还是那一种。换的是现场和用词，不是改成另一类更小的损失。原稿若靠 2 万亿去向未揭留人，新稿第一句必须还是这笔大钱去了哪，不能改成家里利息少了、你不是一个人慌\n")
	b.WriteString("- 第一句必须接住原稿钩子力度，但开场切口必须换。原稿若是对仗打脸，新稿第一句必须还是对仗打脸，两边都得是大白话，用新词。禁止写成更软的解释句或中介口吻。禁止用原稿第一句的同义改写开场\n")
	b.WriteString("- 钩子必须在前 2～3 句内完成，大约 40 字内让人听懂「谁的钱、出了什么事」。原稿故意还没说完的答案，必须在前 3 句内再问一次或钉住，不能先讲完利率、报表、共情再来问钱去哪了\n")
	b.WriteString("- 禁止纯解释句、纯共情句开场。不带新数字、新排除、新问题的句子，不能放在钩子位置，包括「你不是一个人在慌」「这不是正常波动」「这不是网上传的」「报表已经写死了」。「给我三分钟」「比过去三年更值钱」全篇最多一次，且不能顶在钩子前面\n")
	...
	b.WriteString("\n【允许换、但禁止洗没】\n")
	b.WriteString("六个机器零件都要在。钩子类型和未揭晓必须留在前 3 句；中后段段落顺序可以换。必须换的是每个论点下面的例子、画面、人物、现场，以及开场切口。\n")
	...
	b.WriteString("- 钩子类型被换成另一类更小的损失，或原稿未揭晓的答案没有出现在前 3 句内\n")
...
func buildWriterUser(manifest manifestLite, source string) string {
	...
	b.WriteString("钩子类型必须跟原稿第一句同类，未揭晓的答案必须出现在前 3 句内。开场切口必须和原稿第一句不同，换的是现场和用词，不是改成更小的损失。不要套提示词里的示范原句。如果听起来还是原稿换词，或钩子类型换了，或开场在共情、解释、讲概念，就算失败。\n")
