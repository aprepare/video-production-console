...
4. 腰部互动段：中段应有一处互动，明确让观众把四字吉利话打在评论里（如"八方来财"）并顺手点赞，机制说清楚，不许含糊暗示；全篇只此一处。缺了就在中段合适的气口补两三句，多了就合并。
5. 课尾转化段：全文最末一段收完（150～250字）：课名《财富觉醒方法论》、主页橱窗、5块钱这些要素齐全；不吆喝不催付；结尾禁止预告下一条的具体内容，只许通用的关注引导。
6. AI味清除：删换"首先/其次/总而言之/值得注意的是/不难发现/让我们"这类书面串词；讲解员腔、连环排比、空喊口号改成街坊聊天的说法。
7. 发布字段与正文一致：titles 8到12条；short_titles 恰好3条、每条不超过15个字（第1条是视频板面主标题、第2条副标题）；descriptions 恰好3条、每条40到70字；topics 4到5个带#；哪个字段跑题或超格就修哪个字段。
...
func parseReviewerReply(raw string) (reviewerReply, error) {
	...
	if err := json.Unmarshal([]byte(text), &reply); err == nil && reply.Verdict != "" {
		return reply, nil
	}
	if extracted := extractJSONObject(text); extracted != "" {
		if err := json.Unmarshal([]byte(extracted), &reply); err == nil && reply.Verdict != "" {
			return reply, nil
		}
		if repaired := repairUnescapedJSONQuotes(extracted); repaired != extracted {
			if err := json.Unmarshal([]byte(repaired), &reply); err == nil && reply.Verdict != "" {
				return reply, nil
			}
		}
	}
