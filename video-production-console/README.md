...
| [AI 接手说明](docs/AI-HANDOFF.md) | 2026-08-18 进度、能用什么、不要做什么、验证命令、工作区红线 |
...
| [混剪自动选片](docs/operations/montage-catalog-matching.md) | catalog 标签 + 全库向量近邻 + 可选对话点选；改 embedding 须重启 |
...
混剪素材库：电影/B-roll/图片放进 `media_root/originals/{movies,broll,images}/`，设置里填写素材库目录、FFmpeg/FFprobe、视觉与 embedding。首页「素材库 → 开始建库」会扫描、切镜、打标、写向量。建好且有就绪镜头后，点「开始风景混剪」或「开始电影混剪」会**自动从 `catalog.db` 检索**（标签池 + 全库向量近邻），不必手工选片。风景线有库时按口播用财经/B-roll；库空才回退旧风景索引。电影线按口播检索。改 embedding 地址/模型/密钥后必须重启控制台。建库与混剪须用同一向量模型。云机建库用独立程序 `cmd/catalog-builder`（`:2031`）打包，回本机合并。完整电影和原音轨不上云。skill snapshot 声明 `2.0` 的新任务走 v2 计划（B-roll 1.5×）；旧任务继续用自己的 snapshot。未知许可只标 `local_draft_only`。详见 [混剪自动选片](docs/operations/montage-catalog-matching.md)。
