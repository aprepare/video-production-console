...
1. 设置页是否显示生图 API Key 已配置，`image_base_url` 和 `image_model` 是否为空；Key 不会回显属于正常行为。
2. 返回 `409 image_service_not_configured` 表示后端运行时缺 Base URL、模型或 Key，不是文案问题。
3. 返回供应商 HTTP 错误时只记录状态码，不得把 Authorization 或 Key 写入错误、日志或页面。
4. 单张失败时先修改该卡片提示词并使用“重新生成”；保存编辑会把卡片置回 pending，旧图片不再预览或进入 ZIP，不能把旧结果冒充为新提示词产物。
5. 未编辑的 ready 卡片重新生成失败时保留旧图并显示错误；成功后使用新文件替换并清理旧文件。
6. 页面图片 404 时先看卡片是否 ready：pending/failed 返回 404 属于预期；数据库 ready 但文件丢失才属于资产损坏，不要伪造占位图。
7. 打包下载按钮和后端接口都要求全部卡片 ready；ZIP 内按三位序号排序并包含 `manifest.json`。单文件上限 32MB、整包输入上限 512MB；缺图或超限时整次下载失败，不会静默生成不完整压缩包。
8. 若供应商返回图片 URL，后端必须拒绝回环、私网、链路本地和未指定地址，防止服务端请求伪造；不能为了兼容供应商而关闭该保护。
9. 当前兼容请求尺寸为竖版 `1024×1536`、横版 `1536×1024`、方图 `1024×1024`。不要改回 `1024×1365`、`1365×1024` 或 `1024×1792` 这类非标准尺寸字符串；实际图片尺寸仍以响应解码结果为准。
