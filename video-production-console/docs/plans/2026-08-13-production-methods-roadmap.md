...
>
> 更新时间 2026-08-13。方式一、二已上线；方式三、四代码已落地，由 skill snapshot capability 门控，真机三场景仍待用户验收。
...
| 3 | 电影混剪 | 本地电影切镜 + 语义检索匹配文案 | 二创文案 + 火山 TTS | 代码已落地，待真机验收 | `mediacatalog` + `BuildV2`（capability 门控） |
| 4 | 图片视频 | 按词级 SRT 分段逐段生图 + 动效 | 二创文案 + 火山 TTS + 词级 SRT | 代码已落地，待真机验收 | v2 `image_video` 预设 + `imageproject` 适配器 |
