...
func TestNewServerHasDefensiveTimeouts(t *testing.T) {
	...
	if server.ReadHeaderTimeout != 10*time.Second || server.ReadTimeout != 12*time.Minute ||
		server.WriteTimeout != 15*time.Minute || server.IdleTimeout != 3*time.Minute || server.MaxHeaderBytes != 1<<20 {
