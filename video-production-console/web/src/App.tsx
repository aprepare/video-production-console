...
function App() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [a, p] = await Promise.all([fetch('/api/accounts'), fetch('/api/projects')])
  ...
  return <div className="shell">
    <header><div><span className="eyebrow">LOCAL VIDEO OPERATIONS</span><h1>视频生产控制台</h1></div><div className="status"><span className="dot"/>本机服务 · 爆款库共享</div></header>
