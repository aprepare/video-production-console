...
const statusLabels: Record<string, string> = { queued: '排队中', running: '运行中', awaiting_input: '等待回复', resuming: '恢复中', completed: '已完成', failed: '失败', canceled: '已取消', interrupted: '已中断', waiting_input: '等待回复', cancelled: '已取消' }
...
const settingFields: Array<[keyof PublicSettings, string, string]> = [
  ['baokuan_base_url', '爆款库地址', 'http://127.0.0.1:2022'], ['obsidian_vault', 'Obsidian Vault', '本地 Vault 目录'],
  ['topic_cards_dir', '选题卡目录', 'Vault 内选题卡目录'], ['grok_base_url', 'Grok 服务地址', 'OpenAI 兼容 API 地址'],
  ['grok_model', 'Grok 模型', '模型名称'], ['codex_binary_path', 'Codex CLI 路径', 'codex 可执行文件路径'],
  ['media_index_path', '素材索引', '媒体索引文件'], ['media_root', '媒体素材目录', '本地媒体根目录'], ['jianying_root', '剪映草稿目录', '剪映草稿根目录'],
]
...
function App() {
  ...
  const [preview, setPreview] = useState<{ asset: Asset; text?: string } | null>(null)
  ...
  const api = useCallback(async (path: string, init: RequestInit = {}) => {
    ...
    if (response.status === 401) { setAuthenticated(false); setCsrf('') }
    return response
  }, [csrf])
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [a, p] = await Promise.all([api('/api/accounts'), api('/api/projects')])
      if (!a.ok || !p.ok) throw new Error('读取控制台数据失败')
      setAccounts(await a.json()); setProjects(await p.json())
    } catch (error) { setMessage(error instanceof Error ? error.message : '控制台服务尚未连接') } finally { setLoading(false) }
  }, [api])

  const loadDetail = useCallback(async (project: Project) => {
    setDetailLoading(true)
    try {
      const [p, t] = await Promise.all([api(`/api/projects/${project.id}`), api(`/api/tasks?project_id=${project.id}`)])
      if (!p.ok || !t.ok) throw new Error('读取项目详情失败')
      const listed = await t.json() as Task[]
      const fullTasks = await Promise.all(listed.map(async task => {
        const taskResponse = await api(`/api/tasks/${task.id}`)
        return taskResponse.ok ? await taskResponse.json() as Task : task
      }))
      setDetail(await p.json()); setTasks(fullTasks)
    } catch (error) { setMessage(error instanceof Error ? error.message : '读取项目详情失败') } finally { setDetailLoading(false) }
  }, [api])
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setCsrf(me.csrfToken || ''); setAuthenticated(true)
  ...
  const login = async (event: FormEvent) => {
    ...
    if (!response.ok) { setMessage('口令不正确，或稍后再试。'); return }
    const result = await response.json() as { csrf_token?: string }
    setCsrf(result.csrf_token || ''); setPassword(''); setAuthenticated(true); setMessage('')
  }
  const logout = async () => { await api('/api/auth/logout', { method: 'POST' }); setAuthenticated(false); setCsrf('') }
  const visible = useMemo(() => account ? projects.filter(project => project.account_id === account) : projects, [projects, account])
  ...
  const createAccount = async (event: FormEvent) => {
    ...
    if (!response.ok) { setMessage('账号创建失败，请检查名称和背景图。'); return }
    setNewAccount(''); setAccountBackground(null); await load()
  }
  const createProject = async (event: FormEvent) => {
    event.preventDefault(); if (!newProject.trim() || !account) return
    const response = await api('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ account_id: account, title: newProject.trim() }) })
  ...
  const answerTask = async (task: Task) => {
    ...
    const response = await api(`/api/tasks/${task.id}/answer`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ answer: answer.trim() }) })
    if (!response.ok) setMessage('任务回复失败。'); else if (selected) await loadDetail(selected)
  }
  const cancelTask = async (task: Task) => {
    if (!window.confirm('确定取消这个 Codex 任务吗？')) return
    const response = await api(`/api/tasks/${task.id}/cancel`, { method: 'POST' })
    if (!response.ok) setMessage('任务取消失败。'); else if (selected) await loadDetail(selected)
  }
  const uploadAsset = async (event: FormEvent) => {
    event.preventDefault(); if (!selected || !assetFile) return
    const body = new FormData(); body.set('file', assetFile)
    const response = await api(`/api/projects/${selected.id}/assets/${assetType}`, { method: 'POST', body })
    if (!response.ok) setMessage('素材上传失败，请检查文件格式。'); else { setAssetFile(null); await loadDetail(selected) }
  }
  const openAsset = async (asset: Asset) => {
    const url = `/api/assets/${asset.id}/content`
    if (!textAssets.has(asset.type)) { window.open(url, '_blank', 'noopener,noreferrer'); return }
    const response = await api(url)
    if (!response.ok) { setMessage('素材预览读取失败。'); return }
    setPreview({ asset, text: await response.text() })
  }
  const openSettings = async () => {
    const response = await api('/api/settings')
    if (!response.ok) { setMessage('设置读取失败。'); return }
    const next = await response.json() as Settings
    setSettings(next); setSettingsDraft({ ...next.public }); setSecretDraft({ grok_api_key: '', pexels_api_key: '' }); setSettingsOpen(true)
  }
  const saveSettings = async (event: FormEvent) => {
    event.preventDefault(); if (!settingsDraft) return
    const response = await api('/api/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ public: settingsDraft, secrets: secretDraft }) })
    if (!response.ok) { setMessage('设置保存失败，请检查填写内容。'); return }
    const next = await response.json() as Settings
    setSettings(next); setSettingsDraft({ ...next.public }); setSecretDraft({ grok_api_key: '', pexels_api_key: '' }); setMessage('设置已保存。')
  }
  ...
  return <div className="shell">
    ...
    {selected && <div className="drawer-backdrop" onClick={closeProject}><aside className="drawer" onClick={event => event.stopPropagation()}><div className="drawer-head"><div><span className="muted">{accountName(selected.account_id, accounts)}</span><h2>{selected.title}</h2></div><button className="close" onClick={closeProject} aria-label="关闭">×</button></div>{detailLoading && !detail ? <div className="empty">正在读取详情…</div> : detail && <>
      <section className="drawer-section"><h3>项目状态</h3><div className="detail-meta"><span className="stage-badge">{stageLabel(detail.project.stage)}</span><span>更新于 {formatDate(detail.project.updated_at)}</span></div>{detail.missing_assets?.length ? <p className="warning">待补充：{detail.missing_assets.map(item => assetLabels[item] || item).join('、')}</p> : <p className="ok">当前阶段所需素材齐全</p>}</section>
      <section className="drawer-section"><h3>启动工作流</h3><div className="workflow-actions"><button onClick={() => void startTask('topic_select', '给我选题，并在 Obsidian 创建候选选题卡。')}>给我选题</button><button onClick={() => void startTask('topic_deepen', '深化当前选题卡，从爆款库补充依据和可借鉴片段。')}>深化一下</button><button onClick={() => void startTask('remix', '根据当前项目素材完成财经爆款二创。')}>二创文案</button><button onClick={() => void startTask('spoken_format', '把连续版文案转换为口播稿，不删词不漏段。')}>口播稿</button><button onClick={() => void startTask('montage', '使用当前文案、配音、SRT 和固定背景图生成混剪草稿。')}>生成混剪</button></div></section>
      <section className="drawer-section"><h3>素材</h3><form className="asset-upload" onSubmit={uploadAsset}><select value={assetType} onChange={event => setAssetType(event.target.value)}>{Object.entries(assetLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input type="file" onChange={event => setAssetFile(event.target.files?.[0] || null)} /><button disabled={!assetFile}>上传</button></form><div className="asset-list">{Object.entries(detail.assets || {}).map(([type, asset]) => <button className="asset-row" key={type} onClick={() => void openAsset(asset)}><span>{assetLabels[type] || type}</span><div><strong>{asset.filename}</strong><small>{formatSize(asset.size)} · v{asset.version} · 查看</small></div></button>)}{!Object.keys(detail.assets || {}).length && <p className="muted">暂无项目素材</p>}</div></section>
...
function stageLabel(stage: string) { return ({ topic: '选题', script: '文案', assets: '素材', mixing: '混剪', review: '审核', ready: '待发布', published: '已发布' } as Record<string, string>)[stage] || stage }
function accountName(id: string, accounts: Account[]) { return accounts.find(account => account.id === id)?.name || '未分配' }
